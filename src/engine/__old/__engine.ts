...
// import { GroupDisplayMode } from '../grid/reducers/actionTypes'
// import { GridStore } from '../store'
// import { GridColumn} from '../RGridUtils'
// import '../utils'
...
//         var match: GridColumn = _.find(cols, function(column: GridColumn) {
//             return column.sortDirection.length > 0
//         })
...
//                 row.parserValues.forEach(function (myval) {
...
//             var subGroupRows1: any = Enumerable.from(rows).where(x => x[groupColName] == groupColNameValue);
...
//             var dist = enumerableGroupRows.distinct(function (x) { return x[currColumn]; });
//             var currRows = dist.toArray()
//             if (currRows.length == 1)
...
//                             var count: number = subGroupRows.count(function (x) { return x[colName]; });
